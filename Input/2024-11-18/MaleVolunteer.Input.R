...
Male_Volunteer_Variables_df = Variables_df %>% #DF specific for the male volunteer, BDW == 82kg, Hct == 0.467
  filter(age == 67)

Male_Volunteer_Variables_df$BDW_M <- 82 # kg https://doi.org/10.1016/j.envint.2024.109047
Male_Volunteer_Variables_df$Hct_M <- 0.467 # https://doi.org/10.1016/j.envint.2024.109047
Variables_Male_Volunteer_M_df <- Variables_M_df %>% filter(age == 67)

Male_Volunteer_Variables_df = Male_Volunteer_Variables_df %>%
  
  # Final organ volumes
  select(age, ends_with('_M')) %>%
  ...
  mutate(V_boneFraction_M = Male_Volunteer_Variables_df$V_boneFraction_M*BDW_M/2, # 2 is the bone density
         V_bonenonperfusedFraction_M = Male_Volunteer_Variables_df$V_bonenonperfusedFraction_M*BDW_M/2, # 2 is the bone density
         V_adiposeFraction_M = (Male_Volunteer_Variables_df$V_adiposeFraction_M*BDW_M/0.9) + (AdiposeMass_M/0.9), # 0.9 is the bone density
         V_plasmaFraction_M = Male_Volunteer_Variables_df$V_plasmaFraction_M*BDW_M*(1-Male_Volunteer_Variables_df$Hct_M)) %>%  # (1-Hematocrite) corrects for the plasma volume (if not is total blood)
  mutate(TotalVolume_M = rowSums(select(., starts_with("V_")))) %>%

  # Final blood flow to organs
  mutate(TotalBloodFlow_M = Variables_Male_Volunteer_M_df$BloodFlowSum) %>%
  ...
  mutate(TIME = Male_Volunteer_Variables_df$TIME,
         age = Male_Volunteer_Variables_df$age)

colnames(Male_Volunteer_Variables_df) = str_remove(colnames(Male_Volunteer_Variables_df), "Fraction")
write.csv(Male_Volunteer_Variables_df, "MaleVolunteerVariables.csv", row.names = FALSE)
...
# Male
MaleVolunteerPhysioVariables_M_df = Male_Volunteer_Variables_df %>%
...
write.csv(MaleVolunteerPhysioVariables_M_df, "MaleVolunteerPhysioVariablesMale.csv", row.names = FALSE)
...
MaleVolunteerPhysioVariablesMale <- read_csv("MaleVolunteerPhysioVariablesMale.csv")
...
# Male
Final_variables_MVolunteer_df <- MaleVolunteerPhysioVariablesMale %>%
...
write.csv(Final_variables_MVolunteer_df, "Final_variables_MVolunteer.csv", row.names = FALSE)
