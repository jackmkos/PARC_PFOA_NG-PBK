...
VLunc <- Physio_params$V_lungFraction_M
...
VSk <- VSkc * BW
VLun <- VLunc * BW
VR <- (VTotc*BW) - (VA + VG + VK + VL + VP + VSk + VLun)
...
PBPKmodPFOA_M <- function(t, state, parameters){
  with(as.list(c(state, parameters)), {
	...
	CVLun <- CLun/PLun # Concentration leaving lungs (ug/L)
    ...
    # Adipose compartment
    dAA <- QA*(Cart-CVA) # (ug/d)
    
    # Rest compartment
    dAR <- QR*(Cart-CVR) # (ug/d)
    
    # Gut compartment: 
    dAG <- + QG*Cart - QG*CVG + CLbiliary*CL*fuT - CLfecal*CG #Oraldose
    
    # Excretion fecal: cumulative
    dAEx_feces <- CLfecal*CG 
    
    # Liver compartment
    dAL <- QL*Cart + QG*CVG - (QL+QG)*CVL - CLbiliary*CL*fuT
    
    # Kidney compartment
    dAK <- QK*(Cart -CVK) - GFR*fup*CK + CL_FiltPT*CFil*fuFil - CL_PltPT*CK*fuT + CLdif_PTLtPTC*CFil*fuFil - CLdif_PTCtPTL*CK*fuT
    # dAKP <- QK*(Cart - CVKP) - CL_PltPT*fup*CKP + CL_FiltPT*fup*CKT - CL_PltPT*fup*CKP
    ...
    dAart <- - (QSk + QG + QL + QA + QK + QR)*Cart +
	  QC*CVLun
	dAven <- QSk*CVSk + (QL+QG)*CVL + QA*CVA + QK*CVK + QR*CVR -
	  QC*Cven
	
    # Mass Balance
    Atot <- Aart + Aven + ALun + ASk + AG + AL + AA + AK + AFil + AR + AEx_feces + AUr #AKP + AKT
...
#   geom_path(data = output.df, aes(x = Years, y = Cart))+
#   theme_CP()+
...
#   # mutate(CK = CKP + CKT) %>% 
#   select(Years, CK, CSk, CL, CG, CA, CR, Cart, CLun) %>% 
#   rename(Kidney = CK, Skin = CSk, Liver = CL, Gut = CG, Adipose = CA, Rest = CR, Plasma = Cart, Lungs = CLun) %>% 
#   pivot_longer(names_to = "Organ", values_to = "Concentration", Kidney:Lungs) %>% 
#   ggplot()+
...
# PFOA in plasma of one individual
Plot_PFOA_Plasma <- ggplot()+
  geom_path(data = output.PFOA.df, aes(x = Days, y = Cart))+
  theme_CP()+
  # theme(axis.text.x = element_text(size = 7),axis.text.y = element_text(size = 7), axis.title = element_text(size = 8))+
  ylab("Plasma (ng/ml)")
...
Plot_PFOA_All <- output.PFOA.df %>% 
  # mutate(CK = CKP + CKT) %>% 
  select(Days, CK, CSk, CL, CG, CA, CR, Cart, CLun) %>% 
  rename(Kidney = CK, Skin = CSk, Liver = CL, Gut = CG, Adipose = CA, Rest = CR, Plasma = Cart, Lungs = CLun) %>% 
  pivot_longer(names_to = "Organ", values_to = "Concentration", Kidney:Lungs) %>% 
  ggplot()+
...
AUC <- trapz(output_PFOA[ , "time"], output_PFOA[ , "Cart"]) #ug*day/L

# Calculate predicted half-life
time <- output_PFOA[ , "time"] #in days
conc <- output_PFOA[ , "Cart"] #(ug/L) or (ng/ml) 
Cmax <- max(conc)
...
Plot_Plasma <- ggplot()+
  geom_path(data = output.PFOA.df, aes(x = Days, y = Cart), color = "aquamarine")+
