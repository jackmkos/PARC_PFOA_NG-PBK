...
PBPKmodPFOA_M_function <- function(parm.c) {
...
df <- data.frame(mu.star, sigma, parm_names)
dft <- df %>% filter(mu.star >= 0.001)
ggplot(df, aes(x = mu.star, y = sigma)) +
  geom_point() +
  geom_text_repel(aes(label = parm_names), size = 3) +
  theme_minimal() +
  ylab("σ") +
  xlab("μ*")